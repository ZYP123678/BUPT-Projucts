...
class Action:
    ...
    def __init__(self, parser: DSLSparser, input_queue: Queue, output_queue: Queue, is_testing=False):
        self.tree = []  # 存储语法树
        ...
        self.is_testing = is_testing  # 是否在测试模式
    ...
    def initialize_step(self):
        ...
        """
        if not self.tree:
            self.output_queue.put("语法树为空，无法初始化步骤！")
            self.isOver = True
            return
    ...
    def handle_numeric(self, value_str, line):
        ...
        try:
            ...
            self.output_queue.put(line, "Invalid numeric value")
            print(line, "Invalid numeric value")
    ...
    def update_variable(self, line):
        ...
        def replace_var(match):
            ...
            if var_name in self.var:
                ...
                self.output_queue.put(f"未定义的变量：{var_name}")
        ...
        try:
            ...
            self.output_queue.put(f"计算表达式时出错：{line}")
            print(f"计算表达式出错 {line} ")
            return
    ...
    def execute_script(self):
        ...
        while self.current_step is not None:
            # print(f"当前步骤：{self.current_step['Step']}")
            self.has_next_step = False  # 初始化为 False
            for item_dic in self.current_step["proc"]:
                for command, args in item_dic.items():
                    if command == "Run":  # 如果有 Run 指令
            ...
            for item_dic in self.current_step["proc"]:
                for command, args in item_dic.items():
                    if command == "Speak":  # 如果匹配到关键词Speak
                        self.speak = ""
                        for part in args:  # 根据脚本拼接需要输出的Speak
                            if part["type"] == "variable":
                                self.speak += str(self.var.get(part["value"], ""))
                            elif part["type"] == "string":
                                self.speak += part["value"]
                        self.output_queue.put(self.speak)
                    elif command == "Listen":  # 如果匹配到关键词Listen
                        listen_var = args
                        try:
                            self.input = self.input_queue.get(timeout=10)
                        except Empty:
                            self.output_queue.put("您已输入超时，程序自动结束，再见")
                            self.isOver = True
                            return
                        self.var[listen_var] = self.input
                    elif command == "Switch":  # 如果匹配到关键词Switch
                        switch_var = args[0]
                        ...
                        for condition in args[1]:
                            if "Branch" in condition:
                                ...
                                return 
                    elif command == "Run":  # 如果匹配到关键词Run
                        self.execute_run(args)
                        return
                    elif command == "Update":  # 如果匹配到关键词Update
                        update_expr = args
            ...
            if not self.has_next_step and self.current_step["Step"] != self.tree[0]["Step"]:
                ...
                if not self.is_testing:
                    time.sleep(5)
                self.output_queue.put(f"即将跳转回主页面")
    ...
    def execute_run(self, args):
        ...
        next_step = args[0]
        if(next_step == "Exit"):
            self.output_queue.put("您已退出程序，再见")
            self.isOver = True
            return
        # 执行跳转指令
